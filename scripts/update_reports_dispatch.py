    ...
    c.gates = {
      gate3D: {
        ...
        approvers: [
          ...
          { role: '3차: 8D Champion (품질혁신)', name: c.team?.find(t=>t.role.includes('Champion'))?.name || '황승안 팀장_상무', status: 'Approved', date: '2026-09-02 11:00', comment: '전사 초동 대응 종합 승인 및 품질실무 송부 이관' },
          { role: '4차: 8D 품질실무 간사 (송부)', name: '김성중 S.Pro', status: 'Approved', date: '2026-09-02 11:15', comment: '고객사(LGE DTV품질팀) 공식 Initial 3D Report 메일 송부 완결' },
          { role: '5차: 고객사 QA (접수/승인)', name: c.customerContact || '최영수 책임 (LGE)', status: 'Approved', date: '2026-09-02 11:30', comment: '3D 봉쇄 승인 및 고객 DTV 실장 라인 선별 재가동 확인' }
        ]
      },
      gate5D: {
        ...
        approvers: [
          ...
          { role: '4차: 8D 품질실무 간사 (송부)', name: '김성중 S.Pro', status: (c.status === 'Closed' || c.currentStage === 'D8') ? 'Approved' : 'Pending', date: '2026-09-06 16:00', comment: '고객사 기술 대응팀 Interim 5D 정식 송부 완결' },
          { role: '5차: 고객사 QA (기술승인)', name: c.customerContact || '최영수 책임 (LGE)', status: (c.status === 'Closed' || c.currentStage === 'D8') ? 'Approved' : 'Pending', date: '2026-09-06 17:00', comment: 'Root Cause 및 영구대책(PCA) 타당성 동의' }
        ]
      },
      gate8D: {
        ...
        approvers: [
          ...
          { role: '4차: 8D 품질실무 간사 (송부)', name: '김성중 S.Pro', status: (c.status === 'Closed') ? 'Approved' : 'Pending', date: '2026-09-12 18:00', comment: '고객사 품질임원 앞 Final 8D 완결본 공식 송부' },
          { role: '5차: 고객사 최종 서명 및 종결', name: c.customerContact || '최영수 책임 (LGE)', status: (c.status === 'Closed') ? 'Approved' : 'Pending', date: '2026-09-13 10:00', comment: 'LGE 품질보증팀 최종 8D 서명 및 Claim 정식 종결 (Closed)' }
        ]
      }
    };
  }
  ...
  return `
    ...
    <div class="no-print" style="margin-bottom: 16px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
      <div>
        ...
        <p style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 4px;">
          결재 경로: <b>물류/자재책임자 ➔ 8D Leader(연구소) ➔ 8D Champion(품질혁신) [내부승인] ➔ 8D 품질실무간사(김성중) [고객송부] ➔ 고객사 QA [최종서명]</b>
        </p>
      </div>
    ...
    <div class="gate-nav-bar no-print">
      ...
      <div class="gate-nav-card ${currentGateKey === 'gate5D' ? 'active' : ''} ${gates.gate5D.status === 'Approved' ? 'approved' : (gates.gate5D.status === 'Pending' ? 'in-progress' : 'locked')}" onclick="setGateTab('gate5D')">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
          ...
          <span class="badge-pill ${gates.gate5D.status === 'Approved' ? 'badge-ok' : (gates.gate5D.status === 'Pending' ? 'badge-warn' : 'badge-gray')}" style="font-size:0.65rem;">
            ${gates.gate5D.status === 'Approved' ? '● 5D 고객 승인완료' : (gates.gate5D.status === 'Pending' ? '🟡 5D 결재 진행중' : '⚪ 미도달')}
          </span>
        </div>
        <div style="font-size:0.92rem; font-weight:800; color:#f8fafc;">Interim 5D Report</div>
        <div style="font-size:0.72rem; color:var(--text-muted); margin-top:4px;">
          FA리더 ➔ 8D리더 ➔ 챔피언 ➔ <b>김성중(송부)</b> ➔ 고객승인
        </div>
      </div>
      ...
      <div class="gate-nav-card ${currentGateKey === 'gate8D' ? 'active' : ''} ${gates.gate8D.status === 'Approved' ? 'approved' : (gates.gate8D.status === 'Pending' ? 'in-progress' : 'locked')}" onclick="setGateTab('gate8D')">
        ...
        <div style="font-size:0.72rem; color:var(--text-muted); margin-top:4px;">
          8D리더 ➔ R&D부문장 ➔ 챔피언 ➔ <b>김성중(송부)</b> ➔ 고객종결
        </div>
      </div>

    </div>

    <!-- Electronic Approval & Quality Dispatch Panel -->
    <div class="approval-flow-card no-print">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:10px; flex-wrap:wrap; gap:10px;">
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <span style="font-size:0.95rem; font-weight:800; color:#f8fafc;">
            ${activeGate.title} — 단계별 결재 & 고객 송부 현황
          </span>
          <span class="badge-pill ${activeGate.status === 'Approved' ? 'badge-ok' : 'badge-warn'}">
            ${activeGate.status === 'Approved' ? '전체 승인 완결 (Approved)' : (isChampionApproved && !isQualityDispatched ? '내부승인완료 (송부대기)' : '내부 결재 진행 중')}
          </span>
        </div>
      ...
      <!-- Approvers Sequence Grid (5 Columns) -->
      <div style="display:grid; grid-template-columns: repeat(${activeGate.approvers.length}, 1fr); gap:8px;">
        ${activeGate.approvers.map((appr, idx) => `
          <div style="background:#0e172a; border:1px solid ${appr.status === 'Approved' ? '#10b981' : (idx === 3 && isChampionApproved && !isQualityDispatched ? '#f59e0b' : '#334155')}; border-radius:6px; padding:10px 10px; text-align:center; position:relative;">
            <div style="font-size:0.66rem; color:#94a3b8; font-weight:700;">${appr.role}</div>
            <div style="font-size:0.82rem; font-weight:800; color:#f8fafc; margin-top:3px;">${appr.name}</div>
            
            <div style="margin:8px 0;">
              ${appr.status === 'Approved' ? `
                <div class="signoff-stamp-box">
                  ✔ ${idx === 3 ? '송부완료 [송부]' : '승인완료 [인]'}<br>
                  <span style="font-size:9px; font-weight:600;">${appr.date}</span>
                </div>
              ` : (idx === 3 && isChampionApproved ? `
                <div style="border:1px solid #f59e0b; color:#fbbf24; background:rgba(245,158,11,0.1); border-radius:4px; padding:3px 6px; font-size:10px; font-weight:700;">
                  ✉️ 송부 대기 (클릭)
                </div>
              ` : `
            ...
            <div style="font-size:0.67rem; color:#cbd5e1; background:rgba(15,23,42,0.8); border-radius:4px; padding:4px 6px; text-align:left; min-height:36px; line-height:1.2;">
    ...
    <div class="report-paper" id="reportPrintArea">
      ...
      <div class="report-section">
        <div class="report-sec-title">
          <span>${activeGate.title} — Official Approval & Dispatch Flow</span>
          <span>${activeGate.status === 'Approved' ? 'APPROVED' : 'PENDING'}</span>
        </div>
        <div class="report-sec-content">
          <table class="report-inner-table" style="text-align:center;">
            <tr>
              ${activeGate.approvers.map(a => `<th style="width:${100/activeGate.approvers.length}%; font-size:10.5px;">${a.role}</th>`).join('')}
            </tr>
            <tr>
              ${activeGate.approvers.map(a => `
                <td style="padding:8px 3px;">
                  <div style="font-weight:700; font-size:11px; color:#1e3a8a;">${a.name}</div>
                  ${a.status === 'Approved' ? `
                    <div style="color:#059669; font-weight:800; font-size:9.5px; margin-top:4px;">
                      ✔ ${a.role.includes('송부') ? '송부완료' : '서명완료'} (${a.date})
                    </div>
                  ` : `
                    <div style="color:#94a3b8; font-size:9.5px; margin-top:4px;">⏳ 대기</div>
                  `}
                </td>
              `).join('')}
            </tr>
...
function promptSignoffApproval(gateKey) {
  ...
  const pendingApprover = gate.approvers.find(a => a.status !== 'Approved' && !a.role.includes('송부') && !a.role.includes('고객사'));
  if (!pendingApprover) {
    // If internal is done, check customer approval
    const customerApprover = gate.approvers.find(a => a.role.includes('고객사') && a.status !== 'Approved');
    if (customerApprover) {
      if (gate.approvers[3]?.status !== 'Approved') {
        alert('⚠️ 8D Champion 승인 완료 후 [김성중 S.Pro] 님의 고객사 공식 송부가 먼저 실행되어야 합니다!');
        return;
      }
      const comment = prompt(`[고객사 승인] ${customerApprover.name} 님의 승인/접수 의견을 입력하세요:`, '3D 봉쇄 승인 및 라인 정상화 확인');
      if (comment === null) return;
      customerApprover.status = 'Approved';
      customerApprover.date = new Date().toISOString().replace('T', ' ').slice(0, 16);
      customerApprover.comment = comment || '고객사 승인 완료';
      gate.status = 'Approved';
      gate.approvalDate = customerApprover.date;
      saveAppData();
      renderCurrentView();
      alert(`🎉 [${gate.title}] 고객사 최종 승인까지 완결되었습니다!`);
      return;
    }
    alert(`[${gate.title}] 은 이미 모든 결재선(총 ${gate.approvers.length}단계)이 승인 완결되었습니다!`);
    return;
  }
  ...
  if (gate.approvers[2]?.status === 'Approved') {
    ...
    alert(`✔ [8D Champion 황승안 상무] 내부 결재가 최종 승인되었습니다!\\n\\n➔ 이제 [8D 품질실무 김성중 S.Pro] 님에게 이관되어 고객사 송부 버튼이 활성화됩니다.`);
  } else {
...
function dispatchReportToCustomer(gateKey) {
  ...
  const confirmSend = confirm(`[김성중 S.Pro — 8D 품질 실무 간사]\\n\\n내부 결재가 완결된 [${gate.title}] 을(를) 고객사(${c.customer} - ${c.customerContact})로 공식 송부하시겠습니까?`);
  if (!confirmSend) return;

  const nowStr = new Date().toISOString().replace('T', ' ').slice(0, 16);
  dispatchApprover.status = 'Approved';
  dispatchApprover.date = nowStr;
  dispatchApprover.comment = `고객사(${c.customerContact} 앞) 공식 8D Report 이메일 발송 완료`;
  gate.dispatchedByQuality = true;

  saveAppData();
  renderCurrentView();
  alert(`✉️ [김성중 S.Pro] 고객사(${c.customer}) 앞 공식 송부가 성공적으로 완료되었습니다!\\n\\n고객사 접수 및 최종 서명 단계로 전달되었습니다.`);
}
...
with open(os.path.join(base_dir, 'js', 'views', 'reports.js'), 'w', encoding='utf-8') as f:
    f.write(reports_js_updated)
print('Updated reports.js with accurate internal approval -> Quality dispatch -> Customer sign-off flow!')
