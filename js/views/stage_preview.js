...
function createInteractiveExampleCase() {
  ...
  c.d4.selectedTools = toolRows.map(([id,hypothesis,evidence,finding]) => { const row={id,source:'AI',hypothesis,evidence,finding,owner:id==='physical-fa'?'박재환 팀장_S.Pro':'김성중 Senior Pro',status:'Confirmed',verified:true}; row.artifact=typeof createD4EvidenceArtifact==='function'?createD4EvidenceArtifact(id,row,c,true):null; return row; });
...
function renderStageReportSection(c,stage) {
  ...
  if(stage==='D4'){const roots=c.d4?.rootCauses||{};const tools=c.d4?.selectedTools||[];return `<section class="stage-report-section"><h3>D4 · Root Cause Proof & Evidence Package</h3><div class="stage-report-callout">본문 요약 ${tools.length}건 · 뒤쪽에 품질도구별 독립 분석 Evidence ${tools.length}페이지가 첨부됩니다.</div><div class="d4-report-evidence-index">${tools.map((r,i)=>`<div><b>E${String(i+1).padStart(2,'0')}</b><span>${typeof getD4ToolById==='function'?(getD4ToolById(r.id)?.name||r.id):r.id}</span><em class="${r.artifact?.humanConfirmed?'ok':'wait'}">${r.artifact?.humanConfirmed?'VERIFIED':'DRAFT'}</em></div>`).join('')}</div><div class="stage-report-cause-grid">${['Occurrence','Escape','System'].map(type=>`<div><b>${type} Root Cause</b><p>${reportEmpty(roots[type]?.statement)}</p><small>Evidence · ${reportEmpty(roots[type]?.evidence)}</small></div>`).join('')}</div></section>`;}
